...
const [isAnnual, setIsAnnual] = useState(false);
  ...
  return (
    <AnimatePresence>
           <section id="pricing" className="py-20 px-6 bg-gradient-to-b from-purple-900 to-slate-900">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          >
            <span className={`text-lg font-semibold ${!isAnnual ? 'text-white' : 'text-purple-300'}`}>Monthly</span>
            <button
            ...
            <span className={`text-lg font-semibold ${isAnnual ? 'text-white' : 'text-purple-300'}`}>Annual</span>
            {isAnnual && (
              <motion.span
          ...
          {plans.map((plan, idx) => (
            <motion.div
              ...
              className={`relative rounded-2xl p-8 border transition ${
                plan.highlighted
                  ? 'bg-gradient-to-br from-purple-600 to-purple-800 border-purple-400 shadow-2xl shadow-purple-500/50'
                  : 'bg-gradient-to-br from-purple-800 to-slate-800 border-purple-600 border-opacity-50'
              }`}
            >
              {plan.highlighted && (
                <motion.div
                  ...
                  className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-purple-400 to-pink-400 text-slate-900 px-4 py-1 rounded-full text-sm font-bold"
                >
                  MOST POPULAR
                </motion.div>
              )}
              ...
              <div className="space-y-4">
                {plan.features.map((feature, featureIdx) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.4, delay: 0.2 + featureIdx * 0.05 }}
                    className="flex items-start gap-3"
                  >
                    <motion.span
                      className="text-green-400 font-bold mt-1"
                      animate={{ scale: [1, 1.2, 1] }}
                      transition={{ duration: 1.5, repeat: Infinity, delay: featureIdx * 0.1 }}
                    >
                      ✓
                    </motion.span>
        ...
        >
          <p>All plans include 7-day free trial. No credit card required. Cancel anytime.</p>
        </motion.div>
      </div>
    </section>
    </AnimatePresence>
