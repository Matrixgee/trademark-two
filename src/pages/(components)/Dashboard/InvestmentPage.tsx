...
const InvestmentPage = () => {
  ...
  return (
    <div className="p-6 w-full">
        <InvestmentModal plan={planInfo} getPlans={getPlans} selectedCoin={selectedCoin} amount={amount} setAmount={setAmount} handleInvestment={handleInvestment} setIsModalVisible={setIsModalVisible} isModalVisible={isModalVisible} showModal={showModal} handleCancel={handleCancel} loading={loading} setLoading={setLoading}/>
      ...
      <div className="w-full flex flex-col gap-2">
        {assets.map(asset => (
          <div key={asset.id} className="w-full rounded-lg px-2 p-4 gap-1 bg-white flex items-center justify-between shadow-md transition">
            ...
            <div className="">
              <button onClick={()=>{showModal(); setMode(asset.id); setSelectedCoin(asset.name) } } className="bg-purple-600 max-sm:p-2 px-6 py-2 cursor-pointer text-white rounded-lg font-semibold hover:bg-purple-700">
              Invest
            </button>
            </div>
          </div>
